...
MARRIAGE_REGEX = re.compile(r'\*\*(.+)\*\* and \*\*(.+)\*\* are now married!')
...
async def on_message(m: Message):
    ...
    if match := MARRIAGE_REGEX.search(m.content):
        user = match.group(1)
        waifu: str = match.group(2)

        if not all([user, waifu]):
            return
        
        embed: Embed = discord.Embed(title=f"{waifu}", colour=colour.Colour.red(), type="rich", url=m.jump_url)
        cached_character: Union[Embed, None] = CharacterEmbeds.get(waifu)

        if cached_character:
            embed.description = re.sub(r'<:\S+:\d+>', '', re.sub(r'<:kakera:\d+>', ' Ka', str(cached_character.description)))
            embed.set_image(url=cached_character.image.url)
            embed.set_author(name=user)
            embed.add_field(name="\u200B", value=f"[Jump to Message]({m.jump_url})")

        await Channels['Announcements'].send(content=f"**{user}** claimed **{waifu}**!", embed=embed)
